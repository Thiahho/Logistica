...
|---|---|---|---|
| Hoy | Mapa (marcadores numerados por orden + recorrido OSRM) y, debajo, paradas en orden con contador hechas/pendientes. Si la ruta no tiene el retiro confirmado, las paradas no se abren y la tarjeta de próxima parada se reemplaza por la del retiro — el mapa se sigue viendo. | 18, 25, 35 | construida; el gate del retiro es 4.7 |
| Retiro (`/hoy/retiro`) | Bultos esperados (congelados por el sistema contra la lista de la ruta), bultos contados, observación **obligatoria si difieren**, km inicial del odómetro y firma en canvas. Sin esto, `/llegada` y `/cierre` devuelven 409 (acta §7). | 35 | **diseñada, changelog 4.7** |
...
| Cierre de parada | Entregado (foto de la entrega + receptor + posición; con identidad verificada, número de documento y foto del documento) o fallido (motivo de lista cerrada). Al cerrar, cae directo en la parada siguiente, resuelta por el servidor. Escribe directo contra la API; guarda-local-y-sincroniza (cola offline) sigue sin construir. | 20, 21, 23, 24 | construida; el documento y el salto a la siguiente son 4.7 |
| Cierre de jornada (`/hoy/cierre`) | Km final contra el declarado al retirar, combustible, peajes y notas. Es una **declaración pendiente de revisión**, no el cierre de la ruta: queda inmutable y administración la aprueba o la corrige (§4.1, acta RF-26). Habilitada solo cuando no queda ninguna parada pendiente. | 26 | **diseñada, changelog 4.7** |
| Indicador de cola pendiente | — | 25 | **sin construir** |

> **Estado de este documento al 17/09/2026:** las filas marcadas "diseñada, changelog 4.7" y todo
> lo que sigue en esta sección son **diseño acordado, no código existente**. Se escriben acá antes
> de construir, como manda §0 y como se hizo en el changelog 1.20, y el checkpoint final de la
> tanda relee esta sección contra el código y la corrige donde el diseño haya cambiado en el
> camino. El inventario de lo que existe hoy es `estado_implementacion.md`, no esta sección.
