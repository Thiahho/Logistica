...
**Versión 4.4 — Documento consolidado · 15/09/2026**
...
| **4.4** | **15/09/2026** | **Monitor de jornada** (`/jornada`, admin + operación): detalle de ruta en cualquier estado (`/rutas/[id]`, hasta ahora inexistente — una ruta `en_curso` no tenía ninguna pantalla que mostrara sus paradas) y contadores en vivo por estado de ruta y de parada para el día. Entra por §9.3 (ciclo diario: "armado y cierre de ruta, ejecución en calle" ya es función básica) — **no es el tablero de indicadores** (B2/E3, Anexo I §5, sigue fuera de alcance): no calcula ninguna de sus 10 métricas, no acumula ni compara períodos, solo cuenta filas de `rutas`/`ruta_paradas` para una fecha. Mismo encuadre que Cobranza (changelog 4.3). **Actualización por sondeo cada 20 segundos, no push**: el sistema sigue sin infraestructura de tiempo real (ni SignalR, ni WebSockets) y sin scheduler (mismo criterio que el cierre de ciclo de facturación, changelog 4.2) — se agregó el primer polling del frontend (`lib/hooks/useSondeo.ts`), no un canal nuevo. **Dos acciones nuevas sobre una ruta ya `en_curso`:** reasignar repartidor y reordenar las paradas todavía `pendiente` (RF-12, siempre disponible). Le da herramienta a §11.2-5 ("plan de contingencia por ausencia del repartidor") sin cerrar esa decisión, que sigue abierta. **Dos acciones descartadas a propósito, no un olvido:** cambiar el vehículo de una ruta en curso (el tipo de vehículo ya fijó la tarifa congelada al cerrar planificación, acta 3.11 — cambiarlo después desacopla el precio cobrado del vehículo real) y marcar una parada como entregada a mano desde el back-office (salteería la prueba de entrega de RF-20 y dejaría un pedido `Entregado` sin evidencia, contra RF-28 y el criterio de aceptación 5, "historia completa, sin huecos"). **Reasignar y reordenar no dejan registro de auditoría propio**: no hay tabla de eventos de ruta y crearla cruzaría el techo de 17 tablas — gap aceptado, declarado acá para no quedar como un olvido. Cero tablas nuevas, cero migraciones: capa de lectura sobre `rutas`/`ruta_paradas`/`parada_pedidos` y la vista `v_paradas_repartidor`, todas ya pagadas. Sigue en 17. |
