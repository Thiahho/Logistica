...
**Versión 4.26 — Documento consolidado · 24/09/2026**
...
- **RF-45** Inserción de un pedido urgente en una ruta en curso: solo dentro de la ventana de reagrupamiento, solo si desplaza menos de tres paradas pendientes y siempre con recargo de urgencia. Si su destino ya es una parada pendiente de la ruta, se suma a esa parada. El repartidor recibe el aviso en su jornada. *(Changelog 4.26.)*
...
- **Las urgencias entran en una sola ventana**, solo si desplazan menos de tres paradas, siempre con recargo. Una urgencia fuera de ventana no cuesta el recargo no cobrado: cuesta las entregas que llegaron tarde. **Codificada desde el changelog 4.26 (RF-45):** el sistema solo deja insertar una urgencia en una ruta en curso dentro de la ventana (13:00, una hora, valores provisionales configurables), y solo si desplaza como mucho dos paradas pendientes; la urgencia lleva siempre el recargo. Es un caso distinto del delivery ad-hoc de D14, que sigue siendo otro producto.
...
| **4.26** | **24/09/2026** | **Urgencias en una ruta en curso (RF-45): se codifica la regla de §7, que existía desde la v3.0 sin código.** Operación puede agregar un pedido urgente a una ruta que ya salió. **Condiciones:** solo dentro de la ventana de reagrupamiento, y solo si la inserción desplaza como mucho dos paradas pendientes (las que quedan después del punto donde se agrega). El pedido queda marcado urgente, **siempre con recargo**, y su precio se congela igual que al cerrar una planificación. Si el destino ya es una parada pendiente de esa ruta, el pedido se suma a esa parada y no desplaza ninguna (RF-14). La parada agregada queda anclada: "sugerir orden" no la mueve. El repartidor ve la parada nueva en su jornada y un aviso de operación que tiene que marcar como visto. **Valores provisionales, configurables,** con el mismo estatus que `Precio:FactorUrgencia`: la ventana arranca a las 13:00 (§7) y dura una hora; el tope es de dos paradas desplazadas. La duración de la ventana no estaba escrita en ninguna parte y es una decisión de la Empresa. **No cambia:** el delivery ad-hoc (D14) sigue siendo otro producto, sin ventana ni tope. Tampoco hay tabla nueva: la novedad suma el tipo `urgencia`. Detalle técnico en `construccion_v1.md`, changelog 1.41. |
