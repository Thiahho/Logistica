...
| | |
|---|---|
| Controladores API | **24** (suman `LiquidacionesController`, E2/B4, `RangosController`, E2/B3, y `RentabilidadController`, E2/B7 — `construccion_v1.md` 1.35 a 1.37) |
| Endpoints | **146** acciones `[Http*]` en `Controllers/*.cs` — conteo mecánico y reproducible: `grep -cE '^\s*\[Http(Get\|Post\|Put\|Patch\|Delete)' backend/Logistica/Controllers/*.cs`. Siete de B7 (`GET /rentabilidad`, `POST`/`PUT`/`DELETE /costos-fijos[/{id}]`, `POST /costos-fijos/copiar-mes-anterior`, `GET`/`PUT /objetivos-rentabilidad`) sobre las 139 de B3. Seis de B3 (`GET`/`PUT /rangos[/{codigo}]`, `POST /rangos/recalculo/previsualizacion`, `POST /rangos/recalculo`, `GET /clientes/{id}/rango`, `PUT /clientes/{id}/rango/ajuste`) sobre las 133 de B4. Esas eran ocho más que las 125 de changelog 4.18, todas de B4: `GET /rutas/{id}/liquidacion-sugerida`, `GET` y `PUT /parametros-liquidacion[/{tipo}]`, `GET /liquidaciones/previsualizacion`, `POST /liquidaciones`, `GET /liquidaciones`, `GET /liquidaciones/{id}` y `GET /liquidaciones/{id}/csv`. Las 125 de antes eran ocho más que las 117 de changelog 4.10: `GET /localidades/con-zona`, `PUT /localidades/{id}/zona/automatica`, `POST /localidades/recalcular`, `GET /localidades/{id}/precio-sugerido` y `GET /mi-cuenta/localidades/{id}/precio-sugerido` (4.11), `POST /ubicaciones/desde-mapa` y su espejo `POST /mi-cuenta/ubicaciones/desde-mapa` (4.14) y `GET /mi-cuenta/plan-del-dia` (4.12) |
| Pantallas frontend (`page.tsx`) | **42** (suman `/liquidaciones` y `/liquidaciones/[id]`, B4, `/clientes/rangos`, B3, y `/rentabilidad`, B7; recontado con `find frontend/app -name page.tsx`, no adelantado) |
...
### 3.24 Pago al repartidor y liquidación (`LiquidacionesController`, `Servicios/LiquidacionService.cs`, `RutasController.Cerrar`) — E2/B4, acta RF-41 (changelog 4.21), `construccion_v1.md` 1.35

- **Cierre de ruta:** con parámetros vigentes para el tipo de vehículo, el pago al repartidor se calcula (entregas por parada × valor, más bono si el % de éxito llega al mínimo; los fallos no imputables no cuentan) y su desglose queda congelado en `rutas.liq_*`. Pagar otro monto exige motivo. Sin parámetros, se tipea a mano como antes.
- **Liquidaciones:** comprobante por repartidor y período con las rutas cerradas sin liquidar. Inmutable por trigger (la liquidación y el pago de sus rutas).
...
- **Rangos:** cinco fijos (Sin rango, Bronce, Plata, Oro, Empresa). Umbrales y efectos los configura administración en `/tarifas`, sin valores de fábrica.
- **Recálculo:** trimestral, a mano, solo sobre trimestres cerrados (`/clientes/rangos`). Mide envíos, facturación, antigüedad, semanas activas y pagos en término (FIFO), y deja historial inmutable de cada cambio.
- **Ajuste manual (definición F):** ±1 rango con motivo y vencimiento, desde la ficha del cliente. El rango efectivo se deriva del calculado y del ajuste vigente; no se guarda.
- **Efectos:**
  - descuento sobre la tarifa general, congelado con el precio (`pedidos.descuento_rango`);
  - aviso (no bloqueo) de límite de crédito en el alta interna;
  - orden de los pendientes en el armado (nunca de las paradas);
  - el cliente ve su rango y su descuento en "Mi plan".
...
- `docs/acta_sistema.md` v4.23 (reglas de negocio vigentes)
...
- Tercera pasada del 24/09/2026 (E2/B4, `construccion_v1.md` changelog 1.35, acta 4.21): §1, §3.24 nueva, §4, §5 y §6, con `RutasController`, `LiquidacionesController`, `LiquidacionService`, la migración `AgregarLiquidacion` y `frontend/app/{liquidaciones,tarifas,rutas/[id]/cierre}` releídos. Conteos recontados mecánicamente (133 endpoints, 22 controladores, 40 pantallas, 24 entidades / 21 tablas, 20 migraciones). **Verificado** con `curl`/`psql` contra una copia de la base de desarrollo (borrada después) y con el recorrido completo en Chrome headless (390 px y 1280 px): parámetros → cierre con pago calculado y ajuste con motivo → emisión → comprobante. `dotnet test` 19/19, `dotnet build` sin advertencias, `tsc`, `eslint` y `next build` limpios.
- Cuarta pasada del 24/09/2026 (E2/B3 y la corrección del precio vinculante, `construccion_v1.md` changelog 1.36, acta 4.22):
  - **Alcance:** §1, §3.25 nueva, §4, §5 y §6, con `RangosController`, `RangoClienteService`, `Dominio/RangosCliente.cs`, `PrecioService`, `RutasController.CerrarPlanificacion`, `MiCuentaController`, `PedidosController` (alta, detalle y candidatos) y las pantallas de `frontend/app/{tarifas,clientes,mis-envios,pedidos/nuevo,rutas/[id]/armar}` releídos.
